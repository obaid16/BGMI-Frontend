  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-bgmi-border/60 pb-4">
        <div>
          <h1 className="font-display font-black text-2xl text-white uppercase tracking-wide flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-bgmi-gold" /> Official Rulebook Manager
          </h1>
          <p className="text-xs text-slate-400">Edit, add, or reorder tournament rules and anti-cheat policies.</p>
        </div>
      ...
      <div className="space-y-4">
        {rules.map((rule) => (
          <div key={rule.id} className="bg-bgmi-surface border border-bgmi-border rounded-xl p-5 clip-tactical space-y-2">
            ...
            <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line">{rule.content}</p>
          </div>
        ))}
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Tournament Rule" maxWidth="max-w-md">
        <form onSubmit={handleAddRule} className="space-y-4 text-xs">
          <div className="space-y-1">
            <label className="font-bold text-slate-300 uppercase">Rule Heading</label>
            <input
              type="text"
              required
          ...
          <div className="space-y-1">
            <label className="font-bold text-slate-300 uppercase">Rule Body Text</label>
            <textarea
              rows={4}
              required
